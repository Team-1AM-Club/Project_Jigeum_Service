...
class RouteOption(BaseModel):
    ...
    total_duration_minutes: int | None = Field(None, ge=0)
    total_distance_meters: int | None = Field(None, ge=0)
    departure_at: datetime | None = None
    arrival_at: datetime | None = None
    price: int | None = None
...
class Comparison(BaseModel):
    ...
    selected_option_id: str | None = Field(
        None, description="선택된 옵션 ID (사용자 선택 전엔 null)"
    )
...
class Plan(BaseModel):
    ...
    target_arrival_at: datetime = Field(
        ...,
        description="목표 도착 시각 (arrival_deadline - arrival_preference_minutes)",
    )
    ...
    operating_date: str | None = Field(
        None, description="운행일 (막차 운행일, YYYY-MM-DD)"
    )
    arrival_date: str | None = Field(
        None, description="도착 날짜 (자정 넘어 도착 시 다음 날, YYYY-MM-DD)"
    )
    last_journey_supported: bool = Field(
        False,
        description="막차 지원 여부 (데이터 제공자가 막차 운행 정보를 제공하는지)",
    )
...
class ReplanRequest(BaseModel):
    ...
    current_origin_place_id: str | None = Field(
...
class ReplanComparison(BaseModel):
    ...
    previous_plan_preserved: bool = Field(
        True, description="이전 선택이 삭제되지 않고 보존됨"
    )
    previous_plan_valid: bool = Field(
        False, description="이전 경로가 여전히 유효한지 (보장되지 않음)"
    )
