...
SOURCE_SCHEMA = json.loads(
    (
        Path(__file__).resolve().parents[1] / "Docs/api/source-envelope.schema.json"
...
@SOURCE_FORMATS.checks("date-time", raises=ValueError)
def _aware_datetime(value):
    # jsonschema's optional format extras are not required by the MCP SDK.
...
SOURCE_VALIDATOR = Draft202012Validator(SOURCE_SCHEMA, format_checker=SOURCE_FORMATS)
...
def _request(method: str, path: str, *, body=None, params=None) -> dict[str, Any]:
    ...
    if not base:
        return _error(
            "INTERNAL_ERROR", "JIGEUM_API_BASE_URL(/api/v1 포함)을 설정하세요."
        )
    try:
        ...
        return _error(
            "INVALID_ARGUMENT", "JIGEUM_API_TIMEOUT은 0 초과 120 이하 숫자여야 합니다."
        )
    ...
    for attempt in range(2):
        ...
        if (
            ...
            or envelope["status"]
            not in ("ok", "needs_confirmation", "unavailable", "error")
            or (response.is_error and envelope["status"] != "error")
        ):
            return _error(
                "UPSTREAM_RESPONSE_INVALID", "백엔드 응답이 envelope 계약과 다릅니다."
            )
        if not SOURCE_VALIDATOR.is_valid(envelope):
            return _error(
                "UPSTREAM_RESPONSE_INVALID",
                "백엔드 출처 또는 경고가 응답 계약과 다릅니다.",
            )
...
@server.tool(
    description="백엔드 지원 범위와 데이터 상태를 조회한다. is_demo 표시를 사용자에게 전달한다."
)
...
@server.tool(
    description="장소 후보를 검색한다. 사용자가 선택한 실제 place_id를 조건에 사용한다."
)
...
def confirm_trip(
    ...
    if user_confirmed is not True:
        return _error(
            "USER_CONFIRMATION_REQUIRED", "조건 전체에 대한 사용자 확인이 필요합니다."
        )
    return _request(
        "POST",
        f"/conversations/{quote(conversation_id, safe='')}/confirm",
...
def plan_journey(
    ...
    path = (
        "/journeys/plan/last_journey"
        if trip.get("kind") == "last_journey"
        else "/journeys/plan"
    )
    return _request(
        "POST",
...
def replan_journey(
    ...
    if user_confirmed is not True:
        return _error(
            "USER_CONFIRMATION_REQUIRED", "재탐색에 대한 사용자 확인이 필요합니다."
        )
    return _request(
        "POST",
        "/journeys/replan",
